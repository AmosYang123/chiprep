  ...
  async function start(){
    ...
    try{
      ...
      stop=receivePhotos({peer,token,
        ...
        onPhoto:async file=>{
          beforeReceive();
          const url=URL.createObjectURL(file),link=document.createElement('a'),image=document.createElement('img');
          link.href=url;link.download='study-photo-'+Date.now()+'.'+(file.type==='image/png'?'png':file.type==='image/webp'?'webp':'jpg');
          image.src=url;image.alt='Received photo; click to download';link.append(image);
          $('receivedPhotos').hidden=false;$('receivedPhotoList').append(link);previews.push({url,link});
          if(previews.length>5){const old=previews.shift();URL.revokeObjectURL(old.url);old.link.remove();}
          return importPhoto(file);
        }
      });
